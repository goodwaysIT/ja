...
excerpt: "ハードウェア障害、アプリケーションロジックエラー、人的操作ミス、ウイルス、ハッカー攻撃などによるデータ損失を防止するために、バックアップの有効性を確保する。
問題発生時には、関連データを熟練かつ迅速に復旧し、データ損失リスクを最小限に抑える。"
...
## 1. 目的  
ハードウェア障害、アプリケーションロジックエラー、人的操作ミス、ウイルス、ハッカー攻撃などによるデータ損失を防止するために、バックアップの有効性を確保する。  
問題発生時には、関連データを熟練かつ迅速に復旧し、データ損失リスクを最小限に抑える。データ損失ゼロ（Recovery Point Objective, RPO=0）を目指し、データ復旧時間（Recovery Time Objective, RTO）を可能な限り短縮する。  
...
|:------------------------|:---------------------------------------|:--------------|:--------------------|:---------------|
| rman バックアップ      | 有効な rman バックアップ              | 中程度        | 中程度              | 完全       |
| expdp バックアップ     | 有効な expdp バックアップ             | 中程度        | 低                  | ほぼ完全   |
| フラッシュバック スタンバイ | スタンバイDB + フラッシュバック有効    | 最速          | 中程度              | 完全       |
| 遅延適用スタンバイ     | スタンバイDB + 遅延適用設定           | 高速          | 中程度              | 完全       |

### 3.1 ORACLEフラッシュバックを使用した高速リカバリ  
(事前に関連機能/設定の有効化が必要。「緊急データリカバリシナリオ」参照)  
...
RMANを使用した復元とリカバリ（ローカルまたはリモート）。通常はデータベース全体のリカバリに使用。リカバリ速度はバックアップネットワーク帯域幅、ディスク/テープI/Oに依存。  

### 3.3 論理データベースリカバリ (より簡易、手順は省略)
exp/expdp/sqlloader/create table as/rename table を使用した論理バックアップから、imp/impdp/sqlloader/insert into で復元（より柔軟）。
...
注: Oracleフラッシュバックを使用すると、ポイントインタイムメディアリカバリなしで、データベース/オブジェクト/トランザクション/行の過去の状態を表示/復元できる。  
...
部分復元時、RMANは必要なデータファイル/アーカイブログを取得するためにバックアップピース全体を読み取る必要がある。したがって、バックアップピースが小さいほど、復元は高速化される。特に大規模データベースのテープバックアップや単一/少数ファイルの復元時に有効。  
ただし、filespersetの値が小さいと作成されるバックアップピースが増え、バックアップ性能が低下しメンテナンス時間が増加する。これらの要因と復元時間要件を考慮する必要がある。  
```SQL
...
**保持ポリシーを慎重に選択。**  
テープ保持ポリシーおよびバックアップリカバリポリシーを満たすようにする。カタログを使用しない場合、CONTROL_FILE_RECORD_KEEP_TIMEパラメータが保持ポリシーと一致することを確認。
```SQL
SQL> ALTER SYSTEM SET control_file_record_keep_time=21 SCOPE=BOTH; -- レコードを21日間保持
```
これにより、バックアップレコードがコントロールファイルに21日間保持される。  
詳細は以下を参照:  
参照: Note 461125.1 How to ensure that backup metadata is retained in the controlfile when setting a retention policy and an RMAN catalog is NOT used.  

**以下のカタログメンテナンスコマンドを定期的に実行。**  
理由: Delete Obsolete は保持ポリシー外のバックアップを削除する。  
期限切れバックアップを削除しないと、カタログが肥大化しパフォーマンス問題が発生する。  
```SQL
RMAN> DELETE OBSOLETE;
```

理由: Crosscheck はカタログ/コントロールファイルと物理バックアップの一致を確認する。  
バックアップが失われた場合、このコマンドはバックアップピースを "EXPIRED" に設定する。リカバリ開始時にこのバックアップは使用されず、より古いバックアップが使用される。カタログ/コントロールファイル内の期限切れバックアップを削除するには Delete Expired コマンドを使用。  
```SQL
...
### 7.8 コントロールファイル損失の防止  
現在のバックアップ中ではなく、バックアップ終了時にコントロールファイルバックアップを実行し、常に最新のコントロールファイルを確保。  
```SQL
RMAN> CONFIGURE CONTROLFILE AUTOBACKUP ON;
```

### 7.9 リカバリのテスト  
理由: 実際のリカバリを実行せずにリカバリプロセスを学習でき、データファイルの再復元を回避できる。  
```SQL
SQL> RECOVER DATABASE TEST;
```

### 7.10 アーカイブログバックアップでの "DELETE ALL INPUT" の回避  
理由: "delete all input" は1つのアーカイブディレクトリのアーカイブログをバックアップ後、異なるアーカイブディレクトリのすべてのアーカイブログコピーを削除する。一方 "delete input" は、バックアップされたディレクトリ内のアーカイブログのみを削除する。次回のバックアップではアーカイブディレクトリ2のログとアーカイブディレクトリ1の新しいログをバックアップし、バックアップ済みログをすべて削除する。これにより、最後のバックアップ以降のアーカイブログ（バックアップ済みログを含む）がアーカイブディレクトリ2に保持され、最後のバックアップ前の2つのコピーが維持される。  
```SQL
...
**定期的なクロスホストリカバリ訓練を推奨!**  
### 8.1 ブロック破損の検出とリカバリ  
rman-validate コマンドに "check logical" 句を付けて、物理的および論理的な破損についてデータベースを検証。  
以下の例は全データファイルの検証方法:  
```sql
...
**注: undo_retention: undoデータの有効期限。デフォルトは900（15分）。**  
**ただし、この時間内にundoデータが有効であることを保証する前提は、undo表領域に十分な空きがあること。**  
**undo領域が満杯で新規トランザクションが実行されると、undoデータの有効期限に関係なく上書きされる。**  
**ただし、undo領域に余裕があれば、指定時間を超えても上書きされていない限りundoデータは残存するため、フラッシュバック可能（ただし、テーブルレコードの追加/削除/変更前に alter table table_name enable row movement を実行する必要がある。つまり行移動を許可しないと、指定時間後、上書きされていなくてもフラッシュバック不可となり ora-01466 エラーが発生）。**  
...
さらに、フラッシュバックバージョンクエリを使用すると、過去の一定期間におけるレコードの変更履歴を確認できる。  
```sql
...
フラッシュバックトランザクションクエリ機能を使用して、トランザクションによる全変更を確認することも可能。  
同じXIDは同じトランザクションを示す。  
```sql
...
#### 1) 優先方法: ADGスタンバイ側で事前に遅延適用を設定し、誤削除から復元。  
ADG遅延適用設定方法:  
```SQL
...
または  
log_archive_dest_nパラメータを変更し "DELAY=" を使用。例: DELAY=5 (分)、5分遅延を意味。  
```SQL
alter system set log_archive_dest_2='service=standby reopen=60 lgwr async delay=5 valid_for=(online_logfiles,primary_role) db_unique_name=standby' scope=both;
```

ADG遅延スタンバイ機能によるデータ復元。遅延スタンバイはスタンバイログ適用の遅延を意図的に設定し、プライマリDBの操作が即座にスタンバイに適用されるのを防ぐ。これにより、プライマリDBでの誤データ削除時、遅延時間内はスタンバイのデータが削除されないことを保証し、遅延スタンバイDBからデータを復元できる。  
具体的な操作手順:  
...
フラッシュバックでデータベース全体を誤削除前の時点に戻す。この方法の欠点は明らか：データベース全体のデータを同時に戻すため、本番プライマリ環境ではほぼ操作不可能（1テーブルのために全DBデータを戻せない）。したがって、この方法はバックアップ環境に適する。スタンバイDBでフラッシュバック機能が有効な場合、スタンバイDBでフラッシュバック復元を選択可能。  
具体的な操作手順:  
a) 最古フラッシュバック時間を確認:  
```SQL
